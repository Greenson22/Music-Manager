...
class SpotifyWorker(QThread):
    ...
    def __init__(self, client_id, client_secret, query, task_type, limit=50, num_pages=1):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if self.task_type == 'playlist_tracks':
            ...
            elif 'search' in self.task_type:
                all_results = []
                # --- PERBAIKAN DI SINI ---
                search_type_raw = self.task_type.split('_')[1]
                # Konversi dari bahasa Indonesia ke istilah API yang benar
                if search_type_raw == 'lagu':
                    search_type_api = 'track'
                else:
                    search_type_api = 'playlist'
                # -------------------------
                
                for page in range(self.num_pages):
                    offset = page * self.limit
                    results = sp.search(q=self.query, type=search_type_api, limit=self.limit, offset=offset)
                    
                    if search_type_api == 'playlist':
                ...
                self.search_finished.emit(all_results)
